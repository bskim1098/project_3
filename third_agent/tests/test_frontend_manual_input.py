...
class FrontendManualInputRegressionTests(unittest.TestCase):
    @patch("frontend.streamlit_app.ChatOpenAI")
    def test_ce_and_vc_shared_model_uses_gpt_5_4_mini(self, mocked_chat_openai):
        model = make_real_llm()
        self.assertIs(mocked_chat_openai.return_value, model)
        mocked_chat_openai.assert_called_once_with(
            model="gpt-5.4-mini", temperature=0, max_retries=2
        )
