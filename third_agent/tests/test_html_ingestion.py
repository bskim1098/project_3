...
class HTMLIngestionTests(unittest.TestCase):
    ...
    def test_generic_photo_container_does_not_exclude_chart(self):
        html = """
        <html><body><h1>전망 기사</h1><article>
        <p>경제 전망 수치를 설명하는 충분한 길이의 첫 번째 기사 문단입니다.</p>
        <p>주요 지표의 변화와 전망을 설명하는 충분한 길이의 두 번째 문단입니다.</p>
        <div class="photo-group"><figure>
          <img src="/photo/chart.jpg" alt="2025년 수출 증감률 전망">
          <figcaption>2025년 수출 증감률 전망 [연구원 제공]</figcaption>
        </figure></div></article></body></html>
        """
        blocks = extract_text_blocks_from_html(html, "https://news.test/article")
        selected = select_chart_image_candidates(blocks["image_candidates"])
        self.assertEqual(1, len(selected))
