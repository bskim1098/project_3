...
- GraphRAG: 현재 실행 코드에는 적용되지 않았으며, 전체 업무의 60%를 담당하는 1st_agent에 관계·출처 보조 검색으로 도입할 예정이다.
...
```powershell
uv run python -m unittest discover -s 3rd_agent/tests -p test_html_ingestion.py -v
uv run python -m unittest discover -s 3rd_agent/tests -p test_ingestion_outcome.py -v
uv run python -m unittest discover -s 3rd_agent/tests -p test_frontend_manual_input.py -v
uv run python -m unittest discover -s 3rd_agent/tests -p test_streamlit_ui.py -v
uv run python -m unittest discover -s 3rd_agent/guardrails -v
uv run python -m compileall 3rd_agent/third_agent 3rd_agent/tests 3rd_agent/guardrails
